...
export default function JackyLive() {
  const [sit, setSit] = useState({ gpu: 54, cpu: 32, ram: 47, vram: 61 });
  ...
  useEffect(() => {
    ...
    function applyStatus(s) {
      ...
      setSit((prev) => ({
        ...
        vram: g.available && g.mem_used_mb && g.mem_total_mb ? Math.round((g.mem_used_mb / g.mem_total_mb) * 100) : prev.vram,
      }));
    }
    function drift() {
      setSit((p) => {
        const d = (v, lo, hi, step) => Math.max(lo, Math.min(hi, v + (Math.random() - 0.5) * step));
        return { gpu: d(p.gpu, 42, 78, 3.2), cpu: d(p.cpu, 12, 96, 9), ram: d(p.ram, 30, 90, 5), vram: d(p.vram, 40, 94, 4) };
      });
    }
    async function poll() {
      try {
        const [s, a] = await Promise.all([callJacky('status'), callJacky('assessment').catch(() => null)]);
        if (!alive) return;
        applyStatus(s || {});
        setLive(true);
        if (a && a.badge) setVerdict(String(a.badge));
      } catch {
        if (!alive) return;
        setLive(false);
        drift();
  ...
  async function send() {
    ...
    setAsking(true);
    try {
      const r = await callJacky('ask', 'POST', { prompt: text, task_type: 'general' });
      setLog((l) => [
        ...l,
        { who: 'jackie', text: (r && r.response) || '(no response)' },
        { who: 'system', text: `↳ ${(r && (r.route || r.engine)) || 'routed'}${r && r.model ? ' · ' + r.model : ''}` },
      ]);
      setLive(true);
    } catch (e) {
      setLog((l) => [...l, { who: 'system', text: `⚠ backend unreachable (${e.message}) — offline.` }]);
      setLive(false);
    } finally {
      setAsking(false);
    }
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6 text-foreground">
      <div className="mx-auto max-w-5xl space-y-5">
        <header className="flex items-center gap-3">
          ...
          <Badge variant="outline" className={`ml-auto font-mono tracking-widest ${live ? 'border-emerald-500/40 text-emerald-400' : 'border-amber-500/40 text-amber-400'}`}>
            {live ? '● LIVE' : '● DEMO'}
          </Badge>
        </header>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="p-4">
            ...
            <div className={`mt-2 font-mono text-3xl font-bold ${gpuTone(sit.gpu)}`}>{Math.round(sit.gpu)}°C</div>
        ...
        <Card className="flex items-center gap-3 p-4">
          ...
          <span className={`font-mono text-sm font-bold tracking-wider ${tier.tone}`}>{tier.label}</span>
        ...
        <Card className="p-4">
          ...
          <div className="max-h-72 space-y-2 overflow-y-auto rounded-md bg-muted/30 p-3 text-sm">
            {log.map((m, i) => (
              <p key={i} className={`whitespace-pre-wrap break-words ${m.who === 'system' ? 'italic text-muted-foreground' : ''} ${m.who === 'jackie' ? 'text-cyan-300' : ''} ${m.who === 'you' ? 'text-amber-300' : ''}`}>
