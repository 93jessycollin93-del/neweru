    ...
    const fetchEconomyData = async () => {
      try {
        ...
        setStats({
          ...
          tierBreakdown: tiers,
          inflationFactor: 1.0 + (Math.random() - 0.5) * 0.02, // Simulated
        });
    ...
    <div className="min-h-screen pb-24 p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <AdminMetricCard
          ...
          trendPercent={(stats?.inflationFactor * 100 - 100).toFixed(1)}
        />
      </div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {transactions.length > 0 ? (
              transactions.map((txn, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-secondary/50 rounded-lg text-xs">
                  <div>
